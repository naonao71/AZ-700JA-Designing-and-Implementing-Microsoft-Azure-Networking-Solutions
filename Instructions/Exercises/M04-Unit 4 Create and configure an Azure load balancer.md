...
6. **「保存」** をクリックします。
   ![図 7](../media/add-vms-backendpool.png)
    ...
    > **注**:バックエンドプールのデプロイに5分程度かかる場合があります。
...
> myNSG が表示されない場合は、「なし」で構成し、デプロイ後にNSGから「myNSG」を選択し、ネットワークインターフェイスより関連付けます。
...
11. 前の手順の**プライベート IP アドレス** (例: 10.1.2.4) をブラウザーのアドレス バーに入力 (または貼り付け) して、Enter キーを押します。
