...
1. Azure portal ホーム画面で、「Virtual Machines」、「選択」、「**+ 作成**」、「**+ Azure 仮想マシン**」の順に選択します。
...
6. 別の仮想マシンを作成します。手順 2 〜 5 を再度実行しますが、仮想マシンに ContosoWestPrivate という名前を付けて、**Private** サブネットを選択します。この際、NIC ネットワークセキュリティグループは **なし** にします。
