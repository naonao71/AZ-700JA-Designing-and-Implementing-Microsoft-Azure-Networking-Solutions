...
5. 「**作成**」を選択します。
6. 作成した「marketing」共有の右端にある省略メニュー「・・・」をクリックし、「**接続**」をクリックします。

  ![グラフィカル ユーザー インターフェイス、アプリケーション 自動的に生成された説明](../media/new-file-share-1.png)

7. 「**接続**」画面の「**Show Script**」ボタンをクリックし、接続スクリプトをコピーして記録しておきます。
...
-->
1. Virtual Machines の ContosoEastPublic に RDP で接続します。
2. PowerShell を使用して、タスク7で記録したスクリプトを実行します。
3. Azure ファイル共有は Z ドライブにマップされません。これは **ContosoEastPublic** VM が Public サブネットにデプロイされているため、アクセスが拒否されています。Public サブネットには、Azure Storage に対応したサービス エンドポイントがありません。ストレージ アカウントは、Public サブネットではなく Private サブネットからのネットワーク アクセスのみ許可します。
4. インターネットへの送信ができることを確認します。
...
6. コンピューターから、Azure portal から作成したストレージアカウント選択します。
