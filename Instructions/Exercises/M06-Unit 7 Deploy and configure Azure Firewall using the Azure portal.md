...
9. 「**保存**」をクリックします。
   ...
   ![仮想ネットワークの作成 - サブネットの編集](../media/edit-default-subnet-for-firewall.png)
...
1. Azure portal のホーム ページで「**リソースの作成**」を選択し、検索ボックスに「**仮想マシン**」と入力して、表示されたら「**仮想マシン**」を選択します。
...
3. 「**基本**」タブで、次の表の情報を使用して新しい VM を作成します。
   ...
   | イメージ                | **Windows Server 2016 Datacenter - Gen 1**                   |
   | サイズ                 | 「**すべてのサイズを表示**」を選択し、一覧「**B1s**」を選択して、「**選択**」を選択します<br /><br /> **(Standard_B1s-1 vcpu、0.5 GiBメモリ (£8.60/月))** |
   | ユーザー名             | **MyAdmin**                                                  |
   | パスワード             | **TestPa$$w0rd!**                                            |
