...
1. Azure portal ページの「**リソース、サービス、ドキュメントの検索 (G+/)**」で、Front Doors と入力し、結果から「**Front Door と CDN プロファイル**」を選択します。

2. 「Front Door と CDN プロファイル」ページで、「**+ 作成**」を選択します。

3. 「オファリングの比較」で、「Azure Front Door」と「簡易作成」が選択されていることを確認して「Continue to create a Front Door」をクリックし、次の情報を入力または選択します。
   ...
   | 名前            | ユニークな名前を入力します（イニシャルなど）                    |
   | レベル        | Standard                |
   | エンドポイント名 | FDendpoint                      |
   | 配信元の種類     | App Services            |
   | 配信元のホスト名 | 最初に作成したWebアプリの名前を選択                    |
   
4. 「**確認および作成**」を選択し、「**作成**」をクリックします。

5. リソースのデプロイが完了したら、「リソースに移動」をクリックします。

6. Front Door の概要に移動し、「**配信元グループ**」を選択し、「**default-origin-group**」をクリックします。

7. 「配信元グループの更新」で、「**配信元の追加**」をクリックして、2番目に作成したWebアプリを登録します。
